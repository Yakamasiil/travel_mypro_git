...
<head>
    ...
    <script>
        function loadShowBtn(){
            var rid = $("#rid").val();
            //是否被登录人收藏过
            $.ajax({
                type: "get",
                url: "favorite?action=showbtn&rid="+rid,
                dataType:"json",
                success: function(data){
                    ...
                    var showBtn = "";
                    if(data.flag){
                        showBtn = '<a  class="btn already" disabled="disabled"><i class="glyphicon glyphicon-heart-empty"></i>点击收藏</a>';
                    }else{
                        showBtn = '<a class="btn" href="javascript:shoucang('+data.rid+')"><i class="glyphicon glyphicon-heart-empty"></i>点击收藏</a>';
                    }
                    showBtn += '<span>已收藏'+data.count+'次</span>';
                    $("[class=collect]").html(showBtn);
                }
            });
        }
        ...
        function shoucang(rid) {
            $.ajax({
                type: "get",
                url: "favorite?action=addFavorite&rid="+rid,
                dataType:"json",
                success: function(data){
                    if(data){
                        loadShowBtn();
...
<div id="header"></div>
    ...
    <div class="wrap">
        ...
        <div class="prosum_box">
            <input id="rid" type="hidden" value="${router.rid}">
            <dl class="prosum_left">
                <dt>
                    <img alt="" class="big_img" src="${router.rimage}">
                </dt>
                <dd>
            ...
            <div class="prosum_right">
               <p class="pros_title">${router.rname}</p>
                ...
                <div class="pros_price">
                    <p class="price"><strong>¥${router.price}</strong><span>起</span></p>
                    <p class="collect">
